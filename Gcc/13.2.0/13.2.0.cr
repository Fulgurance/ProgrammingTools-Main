class Target < ISM::Software
    ...
    def prepareInstallation
        ...
        if option("Pass1")
            ...
            fileAppendDataFromFile( "#{builtSoftwareDirectoryPath}#{Ism.settings.toolsPath}lib/gcc/#{Ism.settings.chrootTarget}/#{@information.version}/include/limits.h",
                                    mainWorkDirectoryPath + "gcc/limitx.h")
            fileAppendDataFromFile( "#{builtSoftwareDirectoryPath}#{Ism.settings.toolsPath}lib/gcc/#{Ism.settings.chrootTarget}/#{@information.version}/include/limits.h",
                                    mainWorkDirectoryPath + "gcc/glimits.h")
            fileAppendDataFromFile( "#{builtSoftwareDirectoryPath}#{Ism.settings.toolsPath}lib/gcc/#{Ism.settings.chrootTarget}/#{@information.version}/include/limits.h",
                                    mainWorkDirectoryPath + "gcc/limity.h")
        elsif option("Pass2")
            makeSource(["DESTDIR=#{builtSoftwareDirectoryPath}","install"],buildDirectoryPath)
        ...
        if !option("Pass1") && !option("Pass2") && !option("Pass3")
            ...
            moveFile("#{Ism.settings.rootPath}usr/lib/*gdb.py","#{builtSoftwareDirectoryPath}#{Ism.settings.rootPath}usr/share/gdb/auto-load/usr/lib")
    ...
    def install
        ...
        if !option("Pass1") && !option("Pass2") && !option("Pass3")
            ...
            runChownCommand(["-R","root:root","#{Ism.settings.rootPath}usr/lib/gcc/#{Ism.settings.systemArchitecture}-pc-linux-gnu/13.2.0/include"])
            runChownCommand(["-R","root:root","#{Ism.settings.rootPath}usr/lib/gcc/#{Ism.settings.systemArchitecture}-pc-linux-gnu/13.2.0/include-fixed"])
        end
    end
